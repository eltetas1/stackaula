...
import {
  Settings, BookOpen, ClipboardList, Palette, BarChart3, Users,
} from 'lucide-react';
...
export default function MaestroPage() {
  ...
  if (loading) {
    return <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg text-gray-600">Gestiona avisos, tareas y asignaturas del aula</p>
            </div>
          </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-2 sm:grid-cols-6 mb-8">
          ...
          <TabsTrigger value="create-aviso" className="flex items-center gap-2">
            <BookOpen className="h-4 w-4" /><span className="hidden sm:inline">Crear Aviso</span>
          </TabsTrigger>
          <TabsTrigger value="create-tarea" className="flex items-center gap-2">
            <ClipboardList className="h-4 w-4" /><span className="hidden sm:inline">Crear Tarea</span>
          </TabsTrigger>
          <TabsTrigger value="subjects" className="flex items-center gap-2">
            <Palette className="h-4 w-4" /><span className="hidden sm:inline">Asignaturas</span>
          </TabsTrigger>
          <TabsTrigger value="students" className="flex items-center gap-2">
            <Users className="h-4 w-4" /><span className="hidden sm:inline">Alumnos</span>
          </TabsTrigger>
          <TabsTrigger value="manage" className="flex items-center gap-2">
            <Settings className="h-4 w-4" /><span className="hidden sm:inline">Gestionar</span>
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="dashboard" className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <Card><CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium text-gray-600">Total Avisos</CardTitle>
          ...
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2">
              <BarChart3 className="h-5 w-5" /> Actividad Reciente
                  ...
                  {allAvisos.slice(0, 5).map((item) => (
                    <div key={item.id} className="flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex-1">
                        ...
                        {item.subject && <p className="text-sm text-gray-600">{item.subject.name}</p>}
                      </div>
        ...
        <TabsContent value="create-aviso">
          <CreateAvisoForm onCreated={() => { refreshAll(); refreshTareas(); setActiveTab('dashboard'); }} />
        </TabsContent>

        {/* Crear Tarea */}
        <TabsContent value="create-tarea">
          <CreateTareaForm onCreated={() => { refreshAll(); refreshTareas(); setActiveTab('dashboard'); }} />
        </TabsContent>

        {/* Asignaturas */}
        <TabsContent value="subjects"><ManageSubjects /></TabsContent>

        {/* Gestionar Avisos y Tareas */}
        <TabsContent value="manage"><ManageAvisosTareas /></TabsContent>
      </Tabs>
    </div>
  );
}
